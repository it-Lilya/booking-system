...
function click(e) {
  ...
  if (inputs[0].value !== "" && inputs[1].value !== "") {
    ...
    fetch(
      `https://students.netoservices.ru/fe-diplom/routes?from_city_id=${inputs[1].id}&to_city_id=${inputs[0].id}`
    )
      .then((response) => response.json())
      .then((data) => {
    ...
    setInterval(() => {
      if (localStorage.getItem("direction-there")) {
        window.location.pathname = "/booking-system/train-selection";
        // window.location.href = "/booking-system/train-selection";
      }
    }, 100);
  }
  if (inputs[1].value === "") {
