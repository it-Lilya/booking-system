...
export function CardsSorting({ element }) {
  ...
  useEffect(() => {
    ...
    if (dataBack !== null) {
      dataBack.forEach((e) => {
        if (
          e.departure.from.datetime >= element.departure.to.datetime &&
          e.departure.from.railway_station_name ===
            element.departure.to.railway_station_name
        ) {
          setBack(<NewCardDirection back={e} />);
          localStorage.setItem('back-ticket', JSON.stringify(e));
        }
      });
    }
        ...
        <div className='train-destinations'>
          <div className='direction-line'>
            <div className='direction-line-container'>
              <p className='direction-line-city'>
              ...
              <div className='new-card-info'>
                <div className='new-card-first'>
                  <p className='new-card-date'>
                    {converterDate(element.departure.from.datetime)}
                  </p>
                  <p className='new-card-city'>
                    {element.departure.from.city.name}
                  </p>
                  <p className='new-card-station'>
                    {element.departure.from.railway_station_name}
                  </p>
                </div>
                <div className='new-card-second'>
                  <p className='available-time'>
                    {durationTime(element.departure.duration)}
                  </p>
                  <i className='new-card-arrow-right'></i>
                </div>
                <div className='new-card-third'>
                  <p className='new-card-date'>
                    {converterDate(element.departure.to.datetime)}
                  </p>
                  <p className='new-card-city'>
                    {element.departure.to.city.name}
                  </p>
                  <p className='new-card-station new-card-station-to'>
                    {element.departure.to.railway_station_name}
                  </p>
                </div>
              </div>
          ...
          <div className='second-card-ticket'>
            <div className='type-container'>
              <div className='first-types-wagon sedentary-type'>
                <NewElementCard
                  element={sedentary}
                  places={sedentaryPlaces}
                  price={sedentaryPrice}
                ></NewElementCard>
              </div>
              <div className='first-types-wagon seat-type'>
                <NewElementCard
                  ...
                  places={seatPlaces}
                  price={seatPrice}
                  priceUpperSeats={seatSeatUpper}
                  priceLowerSeats={seatSeatLower}
                ></NewElementCard>
              </div>
              <div className='first-types-wagon coupe-type'>
                <NewElementCard
                  ...
                  places={coupePlaces}
                  price={coupePrice}
                  priceUpperSeats={coupeSeatsUpper}
                  priceLowerSeats={coupeSeatsLower}
                ></NewElementCard>
              </div>
